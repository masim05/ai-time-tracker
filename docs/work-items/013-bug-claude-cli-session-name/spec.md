...
- **Transcript** (`~/.claude/projects/<slug>/<sessionId>.jsonl`): a rename is
  ...
  This is the only durable, timestamped source and supports full rename history.
  No `sessionName` / `sessionNameSource` field occurs anywhere in the transcript
  corpus (2980 transcripts scanned).
...
- A malformed, empty, or argument-less rename record must degrade to "no name"
  with a warning diagnostic, never throw.
