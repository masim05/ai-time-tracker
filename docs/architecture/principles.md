...
- **Keep side effects at the edges.** Domain and application layers must not produce side effects (I/O, network, database). Side effects belong in `infrastructure/` and `transport/`.
...
- **Framework and infra independence in domain and application layers.** `domain/` and `application/` must import nothing from HTTP frameworks, ORMs, database drivers, external SDKs, or transport protocols. These concerns belong in `infrastructure/` and `transport/`.
...
- **Explicit cross-module calls.** Cross-module interactions must go through the owning module's public API. Direct table access, internal path imports, and undocumented coupling are forbidden.
- **Database ownership is per module.** Each module owns a declared set of tables. Other modules must not read or write those tables directly; they must call the owning module's application layer.
