...
- **Keep side effects at the edges.** Domain and application layers must not produce side effects (I/O, network, persistence). Side effects belong in `infrastructure/` and `cli/`.
...
- **Module-first, extraction-ready.** Modules are workspace packages from the start. Extraction to a private npm package must require only adapter changes, not business logic changes.
- **Framework and infra independence in domain and application layers.** `domain/` and `application/` must import nothing from CLI frameworks, persistence drivers, OS integrations, or external SDKs. These concerns belong in `infrastructure/` and `cli/`.
